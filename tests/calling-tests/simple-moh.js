...
function onSessionStateChange(event) {
    TestApp.Log("OnSessionStateChange event: " + JSON.stringify(event));
...
TestApp.Log("Session: " + phone.GetSession(sessionId)?.Id + " " + phone.GetSession(sessionId)?.State);
...
while (Date.now() < callDeadline) {
    ...
    TestApp.Log("Call not ended, waiting for 300ms");
    if (callEnded) break;
}
